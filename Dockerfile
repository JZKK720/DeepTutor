...
# FIX: Replace any hardcoded host.docker.internal:8781 with localhost:8001 for SSR
# This fixes the SSR issue where the container tries to connect to host.docker.internal
echo "[Frontend] 🔧 Applying SSR port fix (host.docker.internal:8781 -> localhost:8001)..."
find /app/web/.next -type f \( -name "*.js" -o -name "*.json" \) -exec \
    sed -i 's|host.docker.internal:8781|localhost:8001|g' {} \; 2>/dev/null || true
